...
function KellyColorPicker(cfg) {
  ...
  function initStyleSwitch() {
    ...
    styleSwitch.updateSize = function () {
      this.size = parseInt(
        wheelBlockSize - (wheelBlockSize / 100) * (100 - this.sizePercentage)
      );
    ...
    styleSwitch.draw = function () {
      ...
      this.imageData[method] = canvasHelperCtx.getImageData(
        0,
        0,
        canvasHelper.width,
        canvasHelper.width
  ...
  function initColorSaver(align, selected, color) {
    ...
    colorSaver.updateSize = function () {
      this.width = parseInt(
        wheelBlockSize - (wheelBlockSize / 100) * (100 - this.widthPercentage)
      );
    ...
    colorSaver.draw = function () {
      ...
      if (this.selected) {
        ...
        if (this.align == "left") {
          ...
          canvasHelperCtx.lineTo(
            this.width - this.selectSize * 2,
            this.width - this.selectSize
          );
      ...
      this.imageData = canvasHelperCtx.getImageData(
        0,
        0,
        this.width,
        this.width
      ...
      ctx.drawImage(
        canvasHelper,
        this.position.x + this.paddingX,
        this.position.y + this.paddingY
      );
    };
  ...
  wheel.isDotIn = function (dot) {
    // is dot in circle
    if (
      Math.pow(this.pos.x - dot.x, 2) + Math.pow(this.pos.y - dot.y, 2) <
      Math.pow(this.outerRadius, 2)
    ) {
      if (
        Math.pow(this.pos.x - dot.x, 2) + Math.pow(this.pos.y - dot.y, 2) >
  ...
  alphaSlider.draw = function () {
    ...
    alphaGrd.addColorStop(
      0,
      "rgba(" + aRgb.r + "," + aRgb.g + "," + aRgb.b + ",1)"
    );
    alphaGrd.addColorStop(
      1,
      "rgba(" + aRgb.r + "," + aRgb.g + "," + aRgb.b + ",0)"
    );
  ...
  svCursorMouse.initSvCursor = function () {
    ...
    this.svCursorData =
      "url(" + curImageData + ") " + offset / 2 + " " + offset / 2 + ", auto";
  ...
  function constructor(cfg) {
    ...
    if (cfg.resizeWith) {
      if (
        typeof cfg.resizeWith !== "object" &&
        typeof cfg.resizeWith !== "boolean"
      )
    ...
    if (criticalError) {
      if (typeof console !== "undefined")
        console.error("KellyColorPicker : " + criticalError);
      return;
    }
  ...
  function readColorData(cString, falseOnFail) {
    ...
    if (cString.length <= 7) {
      ...
      if (cString.length == 3) h = cString + cString;
      else if (cString.length == 6) h = cString;
  ...
  function getSvFigureQuad() {
    ...
    quad.updateSize = function () {
      var workD =
        wheel.innerRadius * 2 - wheelCursor.paddingX * 2 - this.padding * 2;
  ...
  function getSvFigureTriangle() {
    ...
    triangle.limitDotPosition = function (dot) {
      ...
      var slope =
        (this.path[0].y - this.path[1].y) / (this.path[0].x - this.path[1].x);
      var minY = Math.ceil(this.path[1].y + slope * (finalX - this.path[1].x));
      slope =
        (this.path[0].y - this.path[2].y) / (this.path[0].x - this.path[2].x);
    ...
    triangle.updateSize = function () {
      ...
      this.outerRadius =
        wheel.innerRadius - wheelCursor.paddingX - this.padding;
  ...
  function getP(point1, line1) {
    ...
    var pr =
      (point1.x - line1[0].x) * (line1[1].x - line1[0].x) +
      (point1.y - line1[0].y) * (line1[1].y - line1[0].y);
    var pt = true;
    var cf = pr / l;
  ...
  function updateInput(manualEnter) {
    ...
    if (!manualEnter) {
      if (a < 1 && inputFormat === "mixed") {
        ...
        if (inputFormat === "hex" || inputFormat === "mixed") input.value = hex;
        else input.value = rgba;
      }
    }
  ...
  this.popUpShow = function (e) {
    ...
    var paddingPopup =
      parseInt(popupStyle.paddingBottom) + parseInt(popupStyle.paddingTop);
    if (paddingPopup <= 0) {
      paddingPopup = 0;
    }
  ...
  this.getColorSaver = function (align) {
    for (var i = 0; i <= colorSavers.length - 1; ++i) {
      if (
        (!align && colorSavers[i].selected) ||
        colorSavers[i].align == align
      ) {
        colorSavers[i].rgb = hexToRgb(colorSavers[i].color);
        colorSavers[i].hsv = rgbToHsv(
          colorSavers[i].rgb.r,
          colorSavers[i].rgb.g,
          colorSavers[i].rgb.b
        );
...
export default KellyColorPicker;
